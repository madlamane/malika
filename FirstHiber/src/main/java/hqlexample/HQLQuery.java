...
public class HQLQuery {
	...
	public void executeProjectionHQL(Session session) {
		Query query = session
				.createQuery("select p.name, p.price from Product p");
		List results = query.list();
	...
	public void executeNamedParametersHQL(Session session) {
		String hql = "from Product where price > :abc";
		Query query = session.createQuery(hql);
		query.setDouble("abc", 25.0);
		List results = query.list();
		displayProductsList(results);
	}
	...
	public void executeOrderHQL(Session session) {
		String hql = "from Product"
				+ " p where p.price>25.0 order by p.price desc";
		Query query = session.createQuery(hql);
		List results = query.list();
		displayProductsList(results);
	}
	...
	public void executeAssociationsHQL(Session session) {
		String hql = "select s.name, p.name, p.price from "
				+ "Product p inner join p.supplier as s";
		Query query = session.createQuery(hql);
		List results = query.list();
		displayObjectsList(results);
	}
	...
	public void executeCountHQL(Session session) {
		String hql = "select min(product.price),"
				+ " max(product.price) from Product product";
		Query query = session.createQuery(hql);
		List results = query.list();
		displayObjectsList(results);
	}
	...
	//	query.addScalar("avgPrice",double);
		List results = query.list();
		displayObjectList(results);
...
/*	public void displaySoftwareList(List list) {
	...
	public static void main(String args[]) throws Exception {
		...
		HQLForRestrictions
		NamedParametersHQL
		ObjectNamedParametersHQL
		PagingHQL
		UniqueResultHQL
		OrderHQL
		OrderTwoPropertiesHQL
		AssociationsHQL
		AssociationObjectsHQL
		FetchAssociationsHQL
		CountHQL
		NamedQuery
		ScalarSQL
		SelectSQL
		...
		DeleteHQL
		*/
		String action = "UniqueResultHQL";
		if (action.equalsIgnoreCase(POPULATE)) {
			example.populate(session);
		} else {
			action = "execute" + action;

			Method method = HQLQuery.class.getMethod(action,
